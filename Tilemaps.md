...
Pixel Vision 8 supports two types of tilemap files: `tilemap.png` and `tilemap.json`. The Workspace Explorer will automatically try to load it in the tilemap editor based on the priority of the file type. If both files exist in a project, the `tilemap.json` file will take priority over the `tilemap.png`. Clicking on either file will launch the Tilemap Tool.

![image alt text](images/LoadingTilemaps_image_0.png)
...
While the `tilemap.png` is an easy way to work with tilemap data, it is very limiting. That’s why PV8 prioritizes the tilemap.json file type. Not only will this file load faster and compress better, but it can also store additional data such as collision flags and color offsets. You can create a new tilemap.json file from the drop-down menu.

![image alt text](images/LoadingTilemaps_image_1.png)

If you have a `tilemap.png` file in your project and load it into the Tilemap Tool, it will automatically be converted into a tilemap.json file when you save.

The `tilemap.json` file is saved as a Tiled formatted file. Tiled is a free tilemap editor but in order to handle the unique tile data Pixel Vision 8 requires, each tile is saved as an object. While tilemaps can be loaded and edited directly in Tiled, they may be more complicated to work with since objects are not easily created with the normal drawing tools.

![image alt text](images/LoadingTilemaps_image_2.png)

Each tile has a custom `colorOffset `and `flagID `which are used by the Tilemap at run-time. The easiest way to work with tilemaps in Pixel Vision 8 will be the built-in tool but for those using external tools, Tiled is another alternative.
